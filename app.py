...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        timeline = helper.monthly_timeline(selected_user , processed_data)
        fig , ax = plt.subplots()
        ...
        daily_timeline = helper.daily_timeline(selected_user, processed_data)
        fig, ax = plt.subplots()
        ...
        with col1:
            ...
            busy_day = helper.week_activity_map(selected_user,processed_data)
            fig , ax = plt.subplots()
            ax.bar(busy_day.index , busy_day.values)
            st.pyplot(fig)
        with col2:
            st.header("Most Busy Month")
            busy_month = helper.month_activity_map(selected_user,processed_data)
            fig , ax = plt.subplots()
        ...
        user_heatmap = helper.activity_heatmap(selected_user,processed_data)
        fig , ax = plt.subplots()
        ...
        #Emoji Analysis
        emoji_df = helper.emoji_helper(selected_user, processed_data)
